//! XChaCha20-Poly1305 Precomputation State
//!
//! This module provides a precomputation state for XChaCha20-Poly1305 encryption and decryption.
...
//! - XChaCha20-Poly1305 has a larger nonce size than ChaCha20-Poly1305, making it safer for
//!   applications that need to encrypt many messages with the same key
//!
//! ## Example Usage
//!
//! ```
//! # use libsodium_rs::crypto_aead::{xchacha20poly1305, xchacha20poly1305_state};
//! # use libsodium_rs::ensure_init;
//! # fn main() -> Result<(), Box<dyn std::error::Error>> {
//! # ensure_init()?;
//! // Create a key and precomputation state
//! let key = xchacha20poly1305::Key::generate();
//! let state = xchacha20poly1305_state::State::from_key(&key)?;
//!
//! // Generate a nonce
...
//! let ciphertext = xchacha20poly1305_state::encrypt_afternm(
//!     message,
...
//!
//! // Decrypt the message
//! let decrypted = xchacha20poly1305_state::decrypt_afternm(
//!     &ciphertext,
//!     Some(additional_data),
//!     &nonce,
//!     &state
...
use crate::crypto_aead::xchacha20poly1305::{Key, Nonce, ABYTES};
use crate::{Result, SodiumError};
use std::convert::TryInto;

/// A precomputation state for XChaCha20-Poly1305 encryption and decryption
///
...
/// Encrypt a message using XChaCha20-Poly1305 with a precomputation state
///
/// This function encrypts a message using the XChaCha20-Poly1305 algorithm with a
...
pub fn encrypt_afternm(
    ...
    if result != 0 {
        return Err(SodiumError::EncryptionError(
            "XChaCha20-Poly1305 encryption failed".into(),
        ));
    }
...
/// Decrypt a message using XChaCha20-Poly1305 with a precomputation state
///
/// This function decrypts a message that was encrypted using the XChaCha20-Poly1305
...
/// * `nonce` - The nonce used for encryption (must be the same as used during encryption)
/// * `state` - The precomputation state to use
///
/// # Returns
...
pub fn decrypt_afternm(
    ...
    if result != 0 {
        return Err(SodiumError::DecryptionError(
            "XChaCha20-Poly1305 authentication failed".into(),
        ));
    }
...
/// Encrypt a message using XChaCha20-Poly1305 with detached authentication tag and precomputation state
///
/// This function encrypts a message using the XChaCha20-Poly1305 algorithm with a
...
/// Decrypt a message using XChaCha20-Poly1305 with detached authentication tag and precomputation state
///
/// This function decrypts a message that was encrypted using the XChaCha20-Poly1305
